...
try {
    ...
    _.forEach(etPartList, function (etPartValue) {
        var currentArr = _.get(dataMap, [_.toString(etPartValue.MATNR), 'etPartValue'], []);
        if (_.isEmpty(currentArr)) {
            _.set(dataMap, [_.toString(etPartValue.MATNR), 'etPartValue'], [etPartValue]);
        } else {
            currentArr.push(etPartValue);
            _.set(dataMap, [_.toString(etPartValue.MATNR), 'etPartValue'], currentArr);
        }
    });

    _.forEach(etmData, function (etmItem) {
        var currentArr = _.get(dataMap, [_.toString(etmItem.MATNR), 'etmItem'], []);
        if (_.isEmpty(currentArr)) {
            _.set(dataMap, [_.toString(etmItem.MATNR), 'etmItem'], [etmItem]);
        } else {
            currentArr.push(etmItem);
            _.set(dataMap, [_.toString(etmItem.MATNR), 'etmItem'], currentArr);
        }
    });

    _.forEach(etrData, function (etrItem) {
        var currentArr = _.get(dataMap, [_.toString(etrItem.MATNR), 'etrItem'], []);
        if (_.isEmpty(currentArr)) {
            _.set(dataMap, [_.toString(etrItem.MATNR), 'etrItem'], [etrItem]);
        } else {
            currentArr.push(etrItem);
            _.set(dataMap, [_.toString(etrItem.MATNR), 'etrItem'], currentArr);
        }
    });

    _.forEach(etbData, function (etbItem) {
        var currentArr = _.get(dataMap, [_.toString(etbItem.MATNR), 'etbItem'], []);
        if (_.isEmpty(currentArr)) {
            _.set(dataMap, [_.toString(etbItem.MATNR), 'etbItem'], [etbItem]);
        } else {
            currentArr.push(etbItem);
            _.set(dataMap, [_.toString(etbItem.MATNR), 'etbItem'], currentArr);
        }
    });
    ...
    _.forEach(dataMap, function (v, k) {
        if (!_.isEmpty(v['etPartValue'])) {
            _.forEach(v['etPartValue'], function (etPartValue) {
                var insertData = {};
        ...
        if (!_.isEmpty(v['etmItem'])) {
            var partTraDataMap = {};
            _.each(v['etmItem'], function (etmItem) {
            ...
            for (var etmCot = 0; etmCot < v['etmItem'].length; etmCot++) {
                var etmValue = v['etmItem'][etmCot];
        ...
        if (!_.isEmpty(v['etrItem'])) {
            var travellerProcessMap = {};
            var tmpMap = {};
            for (var etrCot = 0; etrCot < v['etrItem'].length; etrCot++) {
        ...
        if (!_.isEmpty(v['etbItem'])) {
            for (var etbCot = 0; etbCot < v['etbItem'].length; etbCot++) {
                var etbValue = v['etbItem'][etbCot];
    ...
    MES_DB_QUERY.execSql(
        ...
        where\
            ...
            where\
                mmt.partnumber in ('" + _.keys(dataMap).join("','") + "'))"
    )
    if (MES_DB_QUERY.lastError().isValid()) {
        throw MES_DB_QUERY.lastError().text();
    }
